...
非局部导数 NLS 方程(3.3)的解析解为 $q(x,t) = \frac{(8−8i)(ie^{i(t−3x)}−16e^{i(t−x)})}{(ie^{−2ix}+16)^2}$ ，表示周期性波动解。目标是利用 PTS-PINN 学习该解 $q(x,t)$ 。神经网络架构由一个五层全连接网络组成，每隐藏层有 20 个神经元，采用双曲切线(tanh)激活函数。空间区域 $[−5,5]$ 被离散化为 512 个点，时间区域 $[0,1]$ 被离散化为 100 个点。从 IB 数据中随机提取 $N_{IB} = 300$ 点，$N_f = 10000$ 点作为内区域的配缀点。在 PTS-PINN 方案的背景下，这些训练数据被处理以学习数据驱动的周期波解 $q(x,t)$ 。得到的解显示出的 L2 范数误差为 6.460356e-04，相较于精确解。整个学习过程共计 20,262 次迭代，耗时约 874.7039 秒。相应的动态行为如图 3 所示，说明学习动力学与实际动态之间的误差可以忽略不计。此外，我们还讨论了 PTS-PINN 在不同隐藏层数、神经元、IB 点和配位点下的 L2 相对误差，见表 3 和表 4。
...
这就是呼吸波解。需要强调的是，$r(x,y,t)$ 是一个实函数，这导致了一个额外的损失函数，由下式给出

$$
Loss_r=\frac{1}{N_{IB}}\sum_{i=1}^{N_{IB}}{|\hat{r}(x_{IB}^{i},y_{IB}^{i},t_{IB}^{i};\theta )}-r^i|^2. \tag{3.6}
$$

我们尝试通过 PTS-PINN 技术获取 $q(x,y,t)$ 和 $r(x,y,t)$ 解，采用 6 层全连接神经网络，每层隐藏 40 个神经元，并利用 tanh 激活技术。原始训练数据通过传统的有限差分法获得。这涉及将 x 空间区域 $[−12,12]$ 离散为 64 个点，x 空间区域 $[−8,8]$ 离散为 64 个点，时间区域 $[−4,4]$ 离散为 32 个点。我们从原始 IB 数据中随机选择 $N_{IB} = 3000$ 点，并指定 $N_f = 20000$ 点作为内区域的配点。通过在 PTS-PINN 方案中处理这些精心策划的训练数据，我们成功学习了数据驱动的流氓波解 $q(x,y,t),r(x,y,t)$ 。所得到的解与精确解相比，L2 范数误差为 6.421177e-04 和 2.315614e-03。整个学习过程包含 22,180 次迭代，耗时约 1561.3419 秒。相应的动态行为见图 4 和图 5。表 5 和表 6 中的数据概述了 PTS-PINN 在不同配置下的相对 L2 误差，包括不同隐藏层数、IB 点、配位点和每层神经元数不同。这些表格的持续趋势凸显了 PTS-PINN 整体表现优异。
...
举例来说，我们设 $a = 1$ 和 $b = 1$ ，以展示 PTS-PINN 在学习方程（4.1）中两个参数 a 和 b 的有效性，以及对应的解，由[28]

$$
q(x,y,t)=1-\frac{12it+18}{(x+3y)^2+2t^2+4.5},\quad r(x,y,t)=\frac{-12(x+3y)^2+24t^2+54}{((x+3y)^2+2t^2+4.5)^2}. \tag{4.2}
$$

非局部三波相互作用系统，以参数形式表示为：
...
其中，$x \in [−5,5],t \in [−5,5]$，是我们研究的重点。为了展示 PTS-PINN 的有效性，我们选择公式（4.3）中的示例参数值为 a = 1、b = 2 和 c = 3。我们的目标是展示 PTSPINN 学习这些参数并获得相应解（由[30]给出）的能力
...
通过采用拉丁超立方体抽样，我们可以随机选择 $N_l = 5000$ 作为 IB 值数据，$N_f = 5000$ 作为配音点，生成训练数据集。数据集采用精确的流氓波解（4.2）构建，空间坐标 $(x,y,t)$ 被离散化为 64 个点，分别位于 $[−10,10]×[−10,10]×[−10,10]$ 范围内。在 PTS-PINN 方案中，利用获得的训练数据集，采用 6 层隐神经网络，每层 40 个神经元，预测解 $q(x,y,t),r(x,y,t)$ 以及未知参数 a,b。在无噪声的情况下，数据驱动解 $q(x,y,t),r(x,y,t)$ 分别显示出 2.298788e-04 和 1.894221e-03 的 L2 范数误差。图 6 清晰展示了 $(x,y)$ 平面的三维等高线图和数据驱动流氓波解的二维截面图，图 7 中也展示了 $(x,t)$ 平面的三维等高线图。表 7 展示了非局域（2+1）维 NLS 方程的准确参数，以及使用 PTS-PINN 学习到的参数 a 和 b。值得注意的是，PTS-PINN 展现出了极高准确度地正确识别未知参数的能力，尤其是在训练数据无噪声的情况下。此外，即使存在 0.05 和 0.1 噪声，参数 a 和 b 相关的误差依然可接受，表明预测具有鲁棒性。值得注意的是，噪声对参数误差值有负面影响。

![](./src/31/122.png)

![](./src/31/123.png)

在逆问题的背景下，我们仔细观察未知参数和损耗函数在不同噪声水平下迭代次数的变化。图 8（a）和（b）展示了在不同噪声条件下迭代时未知参数的变化。值得注意的是，我们观察到即使在噪声存在的情况下，PTS-PINN 在大约 300 次迭代后仍能有效学习未知参数。图 8（c）显示了随着迭代次数增加，在不同噪声水平下损失函数的波动。研究结果表明，随着噪声水平的升高，收敛效应逐渐减弱。这强调了噪声对学习过程收敛行为的影响。

![](./src/31/124.png)

### 非局部三波相互作用系统的数值实验

随机选择 $N_l = 1000$ 作为 IB 数据，$N_f = 1000$ 作为搭配点，借助精确的孤子解（4.4），考虑（x， t）∈[−5， 5]×[−5， 5]，分别分为 512 个点和 400 个点，采用一个包含 9 个隐膜层、每层 40 个神经元的 PTS-PINN。该架构用于预测解 $q_1(x,t),q_2(x,t),q_3(x,t)$ 以及未知参数 $a,b,c$ 。预测解的三维图如图 9 所示，图 10 则显示了 $q_1(x,t),q_2(x,t),q_3(x,t)$ 的绝对误差曲面。值得注意的是，绝对误差约为 e3，表明预测具有很高的准确性。表 8 展示了非局部三波相互作用系统的精确参数，以及通过 PTS-PINN 学习过程获得的参数 a、b 和 c。与表 8 中的观察类似，噪声对参数误差值的影响是明显的。然而，即使存在噪声，PTS-PINN 依然保持高度准确性，凸显了该方法的稳健性。

![](./src/31/125.png)

![](./src/31/126.png)

图 11（a）、（b）和（c）分析了在不同噪声水平下，未知参数和损耗函数随迭代次数的变化。从这些图中，我们观察到参数 a 大约需要 200 次迭代后学习，而参数 b 大约需要 600 次迭代，参数 c 大约需要 100 次迭代。与图 8 类似，对于非定域三波相互作用系统，PTS-PINN 即使在存在噪声的情况下也能很好地找到参数。然而，噪声也会对损失函数的收敛效应产生负面影响。
